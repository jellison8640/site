...
`r kable(guilds[1:5, 1:10], caption = "Preview of Guild Data")`
...
```{r shorebird og plots, echo=FALSE}
shore <- ggboxplot(guilds, x = "temporada", y = "shore", 
                   ylab = "Shorebird Count", xlab= "Year",
                   ggtheme = theme_linedraw())

shore1 <- ggboxplot(guilds, x = "ciclo", y = "shore", facet.by = "season",
          panel.labs = list(season=c("Otoño/Primavera", "Invierno")), xlab= "Year",
...
GLMEROverdispersion(glmernb.shore)       ##test for overdisperion
...
`r kable(anova(glmernb.shore, glmernb.shore1, glmernb.shore2, glmernb.shore3, glmernb.shore4))`
...
resid_shore1<-data.frame(
    ...
    response_residuals=guilds$shore-predict(glmernb.shore1,type = "response"),
    predicted=predict(glmernb.shore1,type = "response"),
    observed=guilds$shore)

##plot resids and all that jazz
pr <- ggplot(resid_shore1,aes(x=predicted,y=pearsons_residuals)) + geom_point()
dr <- ggplot(resid_shore1,aes(x=predicted,y=deviance_residuals)) + geom_point()
rr <- ggplot(resid_shore1,aes(x=predicted,y=response_residuals)) + geom_point()
po <- ggplot(resid_shore1,aes(x=predicted,y=observed)) + geom_point()
...
gg.s1 <- ggplot(pr1, aes(x, predicted, colour = group)) + 
...
glmernb.fowl1 <- glmer.nb(waterfowl ~ temporada + season + (1|obs), data=guilds)
...
water1 <- ggboxplot(guilds, x = "temporada", y = "waterfowl", xlab= "Year",
                   ylab = "Count", ggtheme = theme_linedraw())
water1
#labs(title="Count grouped by season") + theme(plot.title = element_text(hjust = 0.5))
water2 <- ggboxplot(guilds, x = "temporada", y = "waterfowl", facet.by = "season", 
                   panel.labs = list(season=c("Otoño/Primavera", "Invierno")), 
                   ylab = "Count", xlab= "Year", ggtheme = theme_linedraw())
water2
...
resid_fowl<-data.frame(
    ...
    response_residuals=guilds$waterfowl-predict(glmernb.fowl2,type = "response"),
    predicted=predict(glmernb.fowl2,type = "response"),
    observed=guilds$waterfowl)
...
gulls.terns1 <- ggboxplot(guilds, x = "temporada", y = "gulls_terns",
                ylab = "Count", xlab= "Year", ggtheme = theme_linedraw())
gulls.terns1

gulls.terns2 <- ggboxplot(guilds, x = "temporada", y = "gulls_terns", 
                          facet.by = "season", xlab= "Year", 
                          panel.labs = list(season=c("Otoño/Primavera", "Invierno")),
                          ylab = "Count", ggtheme = theme_linedraw())
gulls.terns2
```
...
resid_gulls<-data.frame(
    ...
    response_residuals=guilds$gulls_terns-predict(glmernb.gulls,type = "response"),
    predicted=predict(glmernb.gulls,type = "response"),
    observed=guilds$gulls_terns)
...
pel.corm1 <- ggboxplot(guilds, x = "temporada", y = "pel_corm",  
                   ylab = "Count", xlab= "Year", ggtheme = theme_linedraw())
pel.corm1
pel.corm2 <- ggboxplot(guilds, x = "temporada", y = "pel_corm", facet.by = "season", 
                   panel.labs = list(season=c("Otoño/Primavera", "Invierno")), 
                   ylab = "Count", xlab= "Year", ggtheme = theme_linedraw())
pel.corm2
```
...
waders1 <- ggboxplot(guilds, x = "temporada", y = "waders", xlab= "Year",
                  ylab = "Count", ggtheme = theme_linedraw())
waders1

waders2 <- ggboxplot(guilds, x = "temporada", y = "waders", facet.by = "season", 
                   panel.labs = list(season=c("Otoño/Primavera", "Invierno")), 
                  ylab = "Count", xlab= "Year", ggtheme = theme_linedraw())
waders2
```
...
mig <- read_csv("mig_final.csv", col_types = cols(Precip = col_double(),
                                                        season = col_character()), na = "0")
colnames(mig)     ##check that data combined and is complete
```

```{r add ciclo, echo=FALSE}
mig$ciclo <- as.character(mig$temporada)
```
...
migrants1 <- ggboxplot(mig, x = "ciclo", y = "migrants", xlab= "Year", 
                       ylab = "# Individuals", ggtheme = theme_linedraw())

migrants <- ggboxplot(mig, x = "ciclo", y = "migrants", facet.by = "season",
          panel.labs = list(season=c("Otoño/Primavera", "Invierno")), 
          ylab = "# Individuals", xlab= "Year", ggtheme = theme_linedraw())
migrants1
migrants
```
...
resid_mig<-data.frame(
    ...
    response_residuals=mig$migrants-predict(mig.mod1,type = "response"),
...
gg.mig1 <- ggplot(pr.mig1, aes(x, predicted, color=group)) + 
  scale_color_manual(values = c("#3f2d54", "#75b8d1"))
...
ggboxplot(mig, x = "ciclo", y = "residents", ylab = "# Individuals", xlab= "Year",
          ggtheme = theme_linedraw())
residents <- ggboxplot(mig, x = "ciclo", y = "residents", facet.by = "season",
                      panel.labs = list(season=c("Otoño/Primavera", "Invierno")),
                      xlab= "Year", ggtheme = theme_linedraw())
residents
```
...
resid_res<-data.frame(
    ...
    response_residuals=mig$residents-predict(res.mod1,type = "response"),
